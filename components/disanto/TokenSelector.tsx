...
const TokenSelector: React.FC<Props> = ({ open, side, onClose, onSelect, tokens = DEFAULT_TOKENS, account, excludeAddress }) => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (!ok) return
      const subset = list.slice(0, 14)
      const entries: [string, Bal][] = await Promise.all(subset.map(async (t) => {
        try {
          if (t.address === 'native') {
            ...
            const [dec, raw] = await Promise.all([
              typeof t.decimals === 'number' ? t.decimals : getTokenDecimals(t.address),
              getErc20Balance(t.address, account)
             ])
