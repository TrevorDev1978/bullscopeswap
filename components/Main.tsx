...
async function fetchUsdPriceOnce(addrLC: string): Promise<number> {
  ...
  if (_dsInFlight[addrLC]) return _dsInFlight[addrLC]
  _dsInFlight[addrLC] = (async () => {
    try {
      const res = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${addrLC}`)
      const j: any = await res.json()
      const pairs: any[] = Array.isArray(j?.pairs) ? j.pairs : []
      const best = pairs.find(p => String(p?.chainId||'').toLowerCase().includes('pulse')) || pairs[0]
      const p = parseFloat(best?.priceUsd ?? '0')
      const v = Number.isFinite(p) ? p : 0
      _dsPriceCache[addrLC] = { v, ts: Date.now() }
      return v
    } finally {
      delete _dsInFlight[addrLC]
    }
  })()
  return _dsInFlight[addrLC]
}
function useUsdPrice(token: Token) {
  const [price, setPrice] = useState<number>(0)
  const addr = (token.address === 'native' ? WPLS : token.address).toLowerCase()
  useEffect(() => {
    let alive = true
    let timer: any
    const update = async () => {
      const v = await fetchUsdPriceOnce(addr)
      if (alive) setPrice(v)
    }
    const tick = () => {
      if (!alive) return
      if (document.hidden) { timer = setTimeout(tick, 2000); return }
      timer = setTimeout(async () => { await update(); tick() }, DS_TTL)
    }
    update(); tick()
    const onVis = () => { if (!document.hidden) { clearTimeout(timer); update().finally(tick) } }
