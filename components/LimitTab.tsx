...
// ====== Router path helper (you already use this logic in Main.tsx) ======
const ROUTER = '0x6CE485B02Cf97a69D8bAbfe18AF83D6a0c829Dde' // Router02 (adjust if needed)
  ...
  try {
    if ((window as any)?.ethereum) {
      ...
      if (cid === PULSE_CHAIN_HEX) {
        return await (window as any).ethereum.request({ method:'eth_call', params:[{ to: address, data }, 'latest'] }) as string
      }
    }
  } catch{}
  const r = await fetch(RPC_URL, { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ jsonrpc:'2.0', id:1, method:'eth_call', params:[{ to: address, data }, 'latest'] }) })
...
// ====== Reference Price (Dexscreener + pool reserves) ======
const DS_TTL = 60_000
...
async function computePoolMidPrice(tokenIn:string, tokenOut:string){
  // choose most-liquid Pulse pair from Dexscreener, then compute on-chain midprice
  ...
  const res = await ethCall(pairAddr, '0x0902f1ac' /* getReserves() */)
  ...
  if (tokenIn.toLowerCase() === token0.toLowerCase() && tokenOut.toLowerCase() === token1.toLowerCase()) {
    ...
    // not a direct pair; caller should path through WPLS or another hop
    return 0
  }
...
const LimitTab: React.FC = () => {
  ...
  const [targetPrice, setTargetPrice] = useState('') // buy/sell (e.g., BLSEYE per PLS)
  ...
  // reference price via pool midprice (anchored to Dexscreener top Pulse pair)
  ...
  // derive minOut from amountIn*targetPrice
  useEffect(()=>{
  ...
  async function place(){
    ...
    const limit = new (window as any).ethers.Contract(LIMIT_ADDRESS, ABI_LIMIT, signer)

    // small default executor tip (configurable in UI later)
    const tipWei = (sell.address==='native'?  0n : 20000000000000000n) // 0.02 PLS if ERC20 to entice fills

    if (sell.address==='native'){
      const tx = await limit.placeOrderPLS(
        buy.address==='native'? WPLS : buy.address,
        _minOut,
        0, // expiry: GTC for now (add date picker later)
        { value: _amountIn }
      )
      await tx.wait()
    } else {
      // ensure allowance
      const erc20 = new (window as any).ethers.Contract(sell.address, ABI_ERC20, signer)
    ...
    <div className="w-full max-w-xl">
      ...
      <div className="bg-[#20242A] rounded-2xl border border-[#2A2F36] p-4">
        ...
        <div className="flex gap-2">
          <input className="flex-1 bg-black/20 border border-white/10 rounded-xl px-3 py-2 outline-none" placeholder={`${buy.symbol} per ${sell.symbol}`} value={targetPrice} onChange={e=>setTargetPrice(e.target.value)} />
        </div>
      ...
      <div className="actions mt-4">
        <button className="confirm-btn w-full" disabled={!amountIn || !minOut} onClick={place}>Place Limit Order</button>
      </div>
